...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['login']) && isset($_POST['nom']) && isset($_POST['prenom'])) {
    ...
    // PROTECTION DU COMPTE ADMIN - Empêcher toute modification
    if ($user['login'] === 'admin') {
        $error_message = "Le compte administrateur ne peut pas être modifié pour des raisons de sécurité.";
    } else {
    ...
    } // Fermer l'accolade du else (protection admin)
}
...
<main>
    ...
    <form class="formulaire_modification_profil" action="" method="post">
        ...
        <?php if ($user['login'] === 'admin'): ?>
            <div class="admin-protection-notice">
                <h3>⚠️ Compte Administrateur Protégé</h3>
                <p>Pour des raisons de sécurité, le compte administrateur ne peut pas être modifié.</p>
                <p>Tous les champs sont en lecture seule.</p>
            </div>
        <?php endif; ?>

        <label for="login">Login:</label>
        <input type="text" id="login" name="login" value="<?= htmlspecialchars($user['login']) ?>"
            <?= $user['login'] === 'admin' ? 'readonly disabled' : 'required' ?>>
        ...
        <input placeholder="<?= $user['login'] === 'admin' ? 'Modification interdite' : 'Laissez vide pour conserver l\'actuel' ?>"
            type="password" id="password" name="password"
            <?= $user['login'] === 'admin' ? 'readonly disabled' : '' ?>>

        <label for="confirm_password">Confirmez le nouveau mot de passe:</label>
        <input placeholder="<?= $user['login'] === 'admin' ? 'Modification interdite' : 'Confirmez le mot de passe' ?>"
            type="password" id="confirm_password" name="confirm_password"
            <?= $user['login'] === 'admin' ? 'readonly disabled' : '' ?>>

        <input type="submit" value="Valider les modifications"
            <?= $user['login'] === 'admin' ? 'disabled class="disabled-btn"' : '' ?>>
